...
import {
  LayoutDashboard, FileText, PlusSquare, Wallet,
  LogOut, Building2, ChevronRight, Sun, Moon, Languages, Menu, X, TrendingDown
} from 'lucide-react';
...
export default function CompanyLayout() {
  ...
  const navItems = [
    { to: '/company/dashboard', icon: LayoutDashboard, label: t('dashboard') },
    { to: '/company/submit', icon: PlusSquare, label: t('submitApplication') },
    { to: '/company/applications', icon: FileText, label: t('myApplications') },
    { to: '/company/wallet', icon: Wallet, label: t('wallet') },
  ];
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="p-6 border-b dark:border-slate-700/50 border-gray-100">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs dark:text-slate-500 text-gray-400 font-medium">{t('companyPortal')}</p>
          </div>
        </div>
      </div>
      ...
      <div className="mx-4 mt-4">
        <div className={`p-4 rounded-2xl border ${
          isLowBalance
            ? 'bg-gradient-to-br from-red-500/10 to-rose-500/5 border-red-500/30 dark:border-red-500/20'
            : 'bg-gradient-to-br from-emerald-500/10 to-teal-500/5 border-emerald-500/30 dark:border-emerald-500/20'
        }`}>
          <div className="flex items-center justify-between mb-1">
            <p className={`text-xs font-bold uppercase tracking-wider ${isLowBalance ? 'text-red-400' : 'text-emerald-400'}`}>
              {t('walletBalance')}
            </p>
            {isLowBalance && <TrendingDown className="w-3.5 h-3.5 text-red-400" />}
          </div>
          <p className={`text-2xl font-black mt-1 ${isLowBalance ? 'text-red-300' : 'text-emerald-300'}`}>
            {balance.toFixed(2)}
            <span className="text-sm font-medium ms-1 opacity-70">JOD</span>
          </p>
          <p className="text-xs dark:text-slate-500 text-gray-400 mt-2 font-medium">
            {t('feePerApp')}: {user?.markup_price || 3} JOD
          </p>
        </div>
      </div>
      ...
      <div className="p-4 border-t dark:border-slate-700/50 border-gray-100">
        <div className="flex items-center gap-3 mb-3 p-3 rounded-xl dark:bg-slate-700/30 bg-gray-50">
          <div className="w-9 h-9 bg-gradient-to-br from-emerald-500 to-teal-600 rounded-full flex items-center justify-center text-xs font-bold text-white flex-shrink-0 shadow-md">
  ...
  return (
    <div className="flex h-screen dark:bg-slate-950 bg-gray-50 overflow-hidden">
      ...
      {/* Sidebar Mobile */}
      <aside className={`fixed inset-y-0 start-0 z-50 w-64 dark:bg-slate-900 bg-white border-e dark:border-slate-700/50 border-gray-200 flex flex-col transition-transform duration-300 lg:hidden ${sidebarOpen ? 'translate-x-0' : '-translate-x-full rtl:translate-x-full'}`}>
